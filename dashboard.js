...
async function loadDashboardData() {
    try {
        ...
        const surveyResponses = loadFromLocalStorage('surveyResponses') || [];
        
        if (surveyResponses.length > 0) {
            // Process real data from localStorage
            dashboardData = processLocalStorageData(surveyResponses);
        } else {
            // Use mock data if no responses yet
...
function processLocalStorageData(responses) {
    // Calculate averages and statistics from localStorage data
    ...
    for (let i = 1; i <= 21; i++) {
        ...
        const values = responses
            .map(r => parseInt(r[questionKey]))
            .filter(v => !isNaN(v));
        questionAverages[questionKey] = values.length > 0
    ...
    const sectionAverages = {
        section_a_avg: (questionAverages.q1 + questionAverages.q2 + questionAverages.q3) / 3,
        section_b_avg: (questionAverages.q4 + questionAverages.q5 + questionAverages.q6) / 3,
        section_c_avg: (questionAverages.q7 + questionAverages.q8 + questionAverages.q9) / 3,
        section_d_avg: (questionAverages.q10 + questionAverages.q11 + questionAverages.q12) / 3,
        section_e_avg: (questionAverages.q13 + questionAverages.q14 + questionAverages.q15) / 3,
        section_f_avg: (questionAverages.q16 + questionAverages.q17 + questionAverages.q18) / 3
    };
    ...
    return {
        ...
        recent_responses: responses.slice(-10),
        distribution: []
    };
}
